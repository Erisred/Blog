...
Categories and tags are basically the same thing, just by different names. They are gathered, handled, and called using the same methods. The below (tested) code was adapted from the  [Jekyll Tag page](/Blog/web-design/2021/02/15/Jekyll-tags.md) - in fact, the only real change here is that I changed `tag` and `tags` for `category` and `categories`. It otherwise remains virtually untouched.

[Link to a post]({{ site.url }}{% link _posts/2021/2021-02-15-Jekyll-tags.md %})..
