...
Categories and tags are basically the same thing, just by different names. They are gathered, handled, and called using the same methods. The below (tested) code was adapted from the [Jekyll tag page](Jekyll%20tag%20page.md) - in fact, the only real change here is that I changed `tag` for `category` and `tags` for `categories`. It otherwise remains virtually untouched.
